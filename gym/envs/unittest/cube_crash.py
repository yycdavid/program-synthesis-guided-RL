...
class CubeCrash(gym.Env):
    ...
    def reset(self):
        ...
        self.hole_x = self.np_random.randint(
        ...
        while True:
            ...
            if np.linalg.norm(
                    self.wall_color -
                    self.bg_color) < 50 or np.linalg.norm(
                    self.cube_color -
                    self.bg_color) < 50:
                continue
            break
    ...
    def step(self, action):
        ...
        obs[FIELD_H - 5:FIELD_H, self.hole_x - HOLE_WIDTH //
            2:self.hole_x + HOLE_WIDTH // 2 + 1, :] = self.bg_color
        obs[self.cube_y - 1:self.cube_y + 2, self.cube_x -
            1:self.cube_x + 2, :] = self.cube_color
