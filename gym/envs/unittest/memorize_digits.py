...
class MemorizeDigits(gym.Env):
    ...
    def reset(self):
        self.digit_x = self.np_random.randint(
            low=FIELD_W // 5, high=FIELD_W // 5 * 4)
        self.digit_y = self.np_random.randint(
            low=FIELD_H // 5, high=FIELD_H // 5 * 4)
    ...
    def step(self, action):
        ...
        if self.digit == -1:
            ...
            if self.digit == action:
                reward = +1
            done = self.step_n > 20 and 0 == self.np_random.randint(
                low=0, high=5)
        ...
        obs[self.digit_y - 3:self.digit_y + 3,
            self.digit_x - 3:self.digit_x + 3] = digit_img
        self.last_obs = obs
