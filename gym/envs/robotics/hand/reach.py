...
class HandReachEnv(hand_env.HandEnv, utils.EzPickle):
    ...
    def _get_achieved_goal(self):
        goal = [self.sim.data.get_site_xpos(name)
    ...
    def _env_setup(self, initial_qpos):
        ...
        self.palm_xpos = self.sim.data.body_xpos[self.sim.model.body_name2id(
            'robot0:palm')].copy()
    ...
    def _sample_goal(self):
        ...
        finger_names = [
            name for name in FINGERTIP_SITE_NAMES if name != thumb_name]
        ...
        meeting_pos += self.np_random.normal(scale=0.005,
                                             size=meeting_pos.shape)
        ...
        if self.np_random.uniform() < 0.1:
            ...
            # This avoids that the thumb constantly stays near the goal
            # position already.
    ...
    def _render_callback(self):
        ...
        for finger_idx in range(5):
            ...
            site_id = self.sim.model.site_name2id(site_name)
            self.sim.model.site_pos[site_id] = achieved_goal[finger_idx] - \
                sites_offset[site_id]
        self.sim.forward()
