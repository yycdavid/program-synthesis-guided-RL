...
class ManipulateEnv(hand_env.HandEnv):
    def __init__(
        ...
        self.parallel_quats = [rotations.euler2quat(
            r) for r in rotations.get_parallel_rotations()]
    ...
    def _reset_sim(self):
        ...
        # Run the simulation for a bunch of timesteps to let everything settle
        # in.
        for _ in range(10):
    ...
    def _sample_goal(self):
        ...
        if self.target_position == 'random':
            ...
            target_pos = self.sim.data.get_joint_qpos('object:joint')[
                :3] + offset
        elif self.target_position in ['ignore', 'fixed']:
            target_pos = self.sim.data.get_joint_qpos('object:joint')[:3]
        else:
            raise error.Error(
                'Unknown target_position option "{}".'.format(
...
class HandBlockEnv(ManipulateEnv, utils.EzPickle):
    def __init__(
        ...
        ManipulateEnv.__init__(self,
                               ...
                               target_position_range=np.array([(-0.04,
                                                                0.04),
                                                               (-0.06,
                                                                0.02),
                                                               (0.0,
