...
def test_atari_preprocessing_grayscale(env_fn):
    ...
    assert np.allclose(
        obs3, cv2.resize(
            obs1, (84, 84), interpolation=cv2.INTER_AREA))
    obs3_gray = cv2.cvtColor(obs3, cv2.COLOR_RGB2GRAY)
    # the edges of the numbers do not render quite the same in the grayscale,
    # so we ignore them
...
def test_atari_preprocessing_scale(env_fn):
    # arbitrarily chosen number for stepping into env. and ensuring all
    # observations are in the required range
    max_test_steps = 10
