    ...
    * Scale observation: optional
    ...
    def __init__(
        ...
        if grayscale_obs:
            ...
            self.obs_buffer = [
                np.empty(
                    env.observation_space.shape, dtype=np.uint8), np.empty(
        ...
        _low, _high, _obs_dtype = (
            0, 255, np.uint8) if not scale_obs else (
            0, 1, np.float32)
    ...
    def reset(self, **kwargs):
        ...
        noops = self.env.unwrapped.np_random.randint(
            1, self.noop_max + 1) if self.noop_max > 0 else 0
