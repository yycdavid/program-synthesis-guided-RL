...
def quantile_huber_loss(
    ...
    if cum_prob is None:
        ...
        cum_prob = (
            th.arange(
                ...
                dtype=th.float) + 0.5) / n_quantiles
        if current_quantiles.ndim == 2:
            ...
            # broadcastable to (batch_size, n_critics, n_quantiles,
            # n_target_quantiles)
    ...
    pairwise_delta = target_quantiles.unsqueeze(
        -2) - current_quantiles.unsqueeze(-1)
    abs_pairwise_delta = th.abs(pairwise_delta)
    huber_loss = th.where(
        abs_pairwise_delta > 1,
        abs_pairwise_delta - 0.5,
        pairwise_delta ** 2 * 0.5)
    loss = th.abs(cum_prob - (pairwise_delta.detach()
                              < 0).float()) * huber_loss
