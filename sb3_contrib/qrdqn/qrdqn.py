...
class QRDQN(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # Make "n_quantiles" copies of actions, and reshape to (batch_size,
            # n_quantiles, 1).
            ...
            # Compute Quantile Huber loss, summing over a quantile dimension as
            # in the paper.
    ...
    def predict(
        ...
        if not deterministic and np.random.rand() < self.exploration_rate:
            ...
            action, state = self.policy.predict(
                observation, state, mask, deterministic)
        return action, state
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(QRDQN, self)._excluded_save_params() + \
            ["quantile_net", "quantile_net_target"]
