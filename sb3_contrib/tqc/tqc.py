...
class TQC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # We need to sample because `log_std` may have changed between two
            # gradient steps
            if self.use_sde:
            ...
            with th.no_grad():
                ...
                # Make target_quantiles broadcastable to (batch_size,
                # n_critics, n_target_quantiles).
                target_quantiles.unsqueeze_(dim=1)

            # Get current Quantile estimates using action from the replay
            # buffer
            current_quantiles = self.critic(
                replay_data.observations, replay_data.actions)
            # Compute critic loss, not summing over the quantile dimension as
            # in the paper.
