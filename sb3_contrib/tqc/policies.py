...
class Actor(BasePolicy):
    ...
    def get_std(self) -> th.Tensor:
        ...
        assert isinstance(
            self.action_dist, StateDependentNoiseDistribution), msg
...
class Critic(BaseModel):
    ...
    def forward(self, obs: th.Tensor, action: th.Tensor) -> List[th.Tensor]:
        ...
        quantiles = th.stack(tuple(qf(qvalue_input)
                                   for qf in self.q_networks), dim=1)
        return quantiles
...
class TQCPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Callable) -> None:
        ...
        if self.share_features_extractor:
            ...
            critic_parameters = [
                param for name,
                param in self.critic.named_parameters() if "features_extractor" not in name]
        else:
    ...
    def make_critic(
            self,
            features_extractor: Optional[BaseFeaturesExtractor] = None) -> Critic:
