    ...
    def predict(
        ...
        if is_image_space(self.observation_space):
            if not (observation.shape ==
                    self.observation_space.shape or observation.shape[1:] == self.observation_space.shape):
...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self,
                obs: th.Tensor,
                deterministic: bool = False) -> Tuple[th.Tensor,
                                                      th.Tensor,
                                                      th.Tensor]:
        """
    ...
    def _get_action_dist_from_latent(
            self, latent_pi: th.Tensor, latent_sde: Optional[th.Tensor] = None) -> Distribution:
        """
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            # Here mean_actions are the logits (before rounding to get the
            # binary actions)
            return self.action_dist.proba_distribution(
                action_logits=mean_actions)
        elif isinstance(self.action_dist, StateDependentNoiseDistribution):
...
def get_policy_from_name(
        base_policy_type: Type[BasePolicy],
