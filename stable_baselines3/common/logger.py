...
class FormatUnsupportedError(NotImplementedError):
    def __init__(
            self,
            unsupported_formats: Sequence[str],
        ...
        super(
            FormatUnsupportedError, self).__init__(
...
class KVWriter(object):
    ...
    def write(self, key_values: Dict[str, Any], key_excluded: Dict[str,
                                                                   Union[str, Tuple[str, ...]]], step: int = 0) -> None:
        """
...
class HumanOutputFormat(KVWriter, SeqWriter):
    def __init__(self, filename_or_file: Union[str, TextIO]):
        ...
        if isinstance(filename_or_file, str):
            ...
            assert hasattr(
                filename_or_file, "write"), f"Expected file or str, got {filename_or_file}"
    ...
    def write(
        ...
        tag = None
        for (
            key, value), (_, excluded) in zip(
            sorted(
                key_values.items()), sorted(
                key_excluded.items())):
    ...
    def _truncate(cls, string: str, max_length: int = 23) -> str:
        return string[: max_length - 3] + \
            "..." if len(string) > max_length else string
...
def filter_excluded_keys(
    ...
    return {
        key: value for key,
        value in key_values.items() if not is_excluded(key)}
        ...
        def cast_to_json_serializable(value: Any):
            ...
            if hasattr(value, "dtype"):
                if value.shape == () or len(value) == 1:
                    # if value is a dimensionless numpy array or of length 1,
                    # serialize as a float
                    return float(value)
                else:
                    # otherwise, a value is a numpy array, serialize as a list
                    # or nested lists
                    return value.tolist()
            return value
...
def make_output_format(
    ...
    if _format == "stdout":
        ...
        return JSONOutputFormat(
            os.path.join(
                log_dir,
                f"progress{log_suffix}.json"))
    elif _format == "csv":
        return CSVOutputFormat(
            os.path.join(
                log_dir,
                f"progress{log_suffix}.csv"))
...
class Logger(object):
    ...
    def record_mean(self, key: str, value: Any,
        ...
        self.name_to_value[key] = old_val * \
            count / (count + 1) + value / (count + 1)
...
def configure(folder: Optional[str] = None,
              format_strings: Optional[List[str]] = None) -> None:
    """
    ...
    output_formats = [
        make_output_format(
            f,
            folder,
            log_suffix) for f in format_strings]
