...
def _check_image_input(observation_space: spaces.Box) -> None:
    ...
    if np.any(
            observation_space.low != 0) or np.any(
...
def _check_unsupported_spaces(
    ...
    if isinstance(
            observation_space,
            spaces.Dict) and not isinstance(
            env,
            gym.GoalEnv):
        warnings.warn(
...
def _check_obs(obs: Union[tuple,
    ...
    if isinstance(observation_space, spaces.Discrete):
        assert isinstance(
            obs, int), "The observation returned by `{}()` method must be an int".format(method_name)
    elif _is_numpy_array_space(observation_space):
        assert isinstance(
            obs, np.ndarray), "The observation returned by `{}()` method must be a numpy array".format(method_name)
...
def check_env(
    ...
    if warn:
        ...
        if isinstance(
                observation_space,
                spaces.Box) and len(
                observation_space.shape) not in [
                1,
                3]:
            warnings.warn(
    ...
    if _is_numpy_array_space(
            observation_space) and _is_numpy_array_space(action_space):
        _check_nan(env)
