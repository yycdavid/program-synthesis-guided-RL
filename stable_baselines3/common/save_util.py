...
def json_to_data(json_string: str,
                 custom_objects: Optional[Dict[str,
                                               Any]] = None) -> Dict[str,
                                                                     Any]:
    """
...
def open_path_str(path: str, mode: str, verbose: int = 0,
...
def open_path_pathlib(
    ...
    #   with corrections
    # if reading failed with FileNotFoundError, calls open_path_pathlib with
    # suffix
...
def save_to_pkl(path: Union[str,
                            pathlib.Path,
                            io.BufferedIOBase],
                obj: Any,
                verbose: int = 0) -> None:
...
def load_from_pkl(path: Union[str,
                              pathlib.Path,
                              io.BufferedIOBase],
                  verbose: int = 0) -> Any:
    """
...
def load_from_zip_file(
    ...
    try:
        with zipfile.ZipFile(load_path) as archive:
            ...
            # files store state_dicts of variables with custom names (e.g.
            # policy, policy.optimizer)
            pth_files = [file_name for file_name in namelist if os.path.splitext(file_name)[
                1] == ".pth"]
