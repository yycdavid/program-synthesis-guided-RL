...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(
        ...
        if isinstance(self.observation_space, gym.spaces.Dict):
            ...
            self.obs_rms = {
                key: RunningMeanStd(
                    shape=space.shape) for key,
                space in self.obs_spaces.items()}
        else:
    ...
    def __getstate__(self) -> Dict[str, Any]:
        ...
        # these attributes depend on the above and so we would prefer not to
        # pickle
        del state["ret"]
        return state
    ...
    def _normalize_obs(
        ...
        """
        return np.clip((obs - obs_rms.mean) / np.sqrt(obs_rms.var +
                                                      self.epsilon), -self.clip_obs, self.clip_obs)
            ...
            if isinstance(obs, dict) and isinstance(self.obs_rms, dict):
                for key in self.obs_rms.keys():
                    obs_[key] = self._normalize_obs(
                        obs[key], self.obs_rms[key]).astype(
                        np.float32)
            else:
                obs_ = self._normalize_obs(
                    obs, self.obs_rms).astype(
                    np.float32)
        return obs_
