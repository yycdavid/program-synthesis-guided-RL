...
class VecFrameStack(VecEnvWrapper):
    ...
    def __init__(self, venv: VecEnv, n_stack: int,
        ...
        assert isinstance(
            wrapped_obs_space, spaces.Box), "VecFrameStack only work with gym.spaces.Box observation space"
        ...
        if channels_order is None:
            ...
            assert channels_order in {
                "last", "first"}, "`channels_order` must be one of following: 'last', 'first'"
    ...
    def step_wait(self) -> Tuple[np.ndarray,
        ...
        if self.channels_first:
            ...
            self.stackedobs[..., -
                            observations.shape[self.stack_dimension]:] = observations
