...
class Distribution(ABC):
    ...
    def proba_distribution_net(
            self, *args, **kwargs) -> Union[nn.Module, Tuple[nn.Module, nn.Parameter]]:
    ...
    def log_prob_from_params(
            self, *args, **kwargs) -> Tuple[th.Tensor, th.Tensor]:
        """
...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self,
                               latent_dim: int,
                               log_std_init: float = 0.0) -> Tuple[nn.Module,
                                                                   nn.Parameter]:
        """
    ...
    def log_prob_from_params(self,
                             mean_actions: th.Tensor,
                             log_std: th.Tensor) -> Tuple[th.Tensor,
                                                          th.Tensor]:
        """
...
class CategoricalDistribution(Distribution):
    ...
    def proba_distribution(
            self,
            action_logits: th.Tensor) -> "CategoricalDistribution":
...
class MultiCategoricalDistribution(Distribution):
    ...
    def proba_distribution(
            self,
            action_logits: th.Tensor) -> "MultiCategoricalDistribution":
        self.distributions = [
            Categorical(
                logits=split) for split in th.split(
                action_logits,
                tuple(
                    self.action_dims),
                dim=1)]
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        # Extract each discrete action and compute log prob for their
        # respective distributions
        return th.stack([dist.log_prob(action) for dist, action in zip(
            self.distributions, th.unbind(actions, dim=1))], dim=1).sum(dim=1)

    def entropy(self) -> th.Tensor:
        return th.stack([dist.entropy()
                         for dist in self.distributions], dim=1).sum(dim=1)

    def sample(self) -> th.Tensor:
        return th.stack([dist.sample() for dist in self.distributions], dim=1)

    def mode(self) -> th.Tensor:
        return th.stack([th.argmax(dist.probs, dim=1)
                         for dist in self.distributions], dim=1)
...
class BernoulliDistribution(Distribution):
    ...
    def proba_distribution(
            self,
            action_logits: th.Tensor) -> "BernoulliDistribution":
...
class StateDependentNoiseDistribution(Distribution):
    ...
    def get_noise(self, latent_sde: th.Tensor) -> th.Tensor:
        ...
        if len(latent_sde) == 1 or len(
                latent_sde) != len(self.exploration_matrices):
...
def make_proba_distribution(action_space: gym.spaces.Space,
                            use_sde: bool = False,
                            dist_kwargs: Optional[Dict[str,
                                                       Any]] = None) -> Distribution:
    """
