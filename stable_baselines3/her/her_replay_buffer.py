...
class HerReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        self.buffer = {
            key: np.zeros(
                (self.max_episode_stored,
                 self.max_episode_length,
                 *dim),
                dtype=np.float32) for key,
            dim in input_shape.items()}
        # Store info dicts are it can be used to compute the reward (e.g.
        # continuity cost)
        self.info_buffer = [
            deque(
                maxlen=self.max_episode_length) for _ in range(
                self.max_episode_stored)]
        # episode length storage, needed for episodes which has less steps than
        # the maximum length
        self.episode_lengths = np.zeros(
            self.max_episode_stored, dtype=np.int64)
    ...
    def set_env(self, env: ObsDictWrapper) -> None:
        ...
        if self.env is not None:
            raise ValueError(
                "Trying to set env of already initialized environment.")
    ...
    def sample_offline(
        ...
        """
        # env=None as we should store unnormalized transitions, they will be
    ...
    def sample_goals(
        ...
        if self.goal_selection_strategy == GoalSelectionStrategy.FINAL:
            ...
            # replay with random state which comes from the same episode and
            # was observed after current transition
            ...
            # replay with random state which comes from the same episode as
            # current transition
            transitions_indices = np.random.randint(
                self.episode_lengths[her_episode_indices])
        ...
        return self.buffer["achieved_goal"][her_episode_indices,
                                            transitions_indices]
    ...
    def _sample_transitions(
        ...
        if online_sampling:
            ...
            # Do not sample the episode with index `self.pos` as the episode is
            # invalid
            if self.full:
                episode_indices = (
                    np.random.randint(
                        ...
                        batch_size) + self.pos) % self.n_episodes_stored
            else:
            ...
            # we sample n_sampled_goal per timestep in the episode (only one is
            # stored).
        ...
        if online_sampling:
            ...
            if her_indices.size == 0:
                ...
                # Now with the corrected episode length when using "future"
                # strategy
                transitions_indices = np.tile(
                    np.arange(ep_lengths[0]), n_sampled_goal)
                episode_indices = episode_indices[transitions_indices]
                her_indices = np.arange(len(episode_indices))

        # get selected transitions
        transitions = {key: self.buffer[key][episode_indices, transitions_indices].copy(
        ) for key in self.buffer.keys()}

        # sample new desired goals and relabel the transitions
        new_goals = self.sample_goals(
            episode_indices, her_indices, transitions_indices)
