...
class HER(BaseAlgorithm):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # compute ratio between HER replays and regular replays in percent for
        # online HER sampling
        ...
        # for online sampling, it replaces the "classic" replay buffer
        # completely
    ...
    def _setup_model(self) -> None:
        ...
        # assign episode storage to replay buffer when using online HER
        # sampling
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.num_timesteps > 0 and self.num_timesteps > self.learning_starts and self.replay_buffer.size() > 0:
                ...
                # do as many gradients steps as steps performed during the
                # rollout
        ...
        while total_steps < n_steps or total_episodes < n_episodes:
            ...
            if done or self.episode_steps >= self.max_episode_length:
                if self.online_sampling:
                    ...
                    # sample virtual transitions and store them in replay
                    # buffer
    ...
    def load(
        ...
        **kwargs,
        ...
        # check if observation space and action space are part of the saved
        # parameters
        ...
        if env is not None:
            ...
            # Use stored env, if one exists. If not, continue as is (can be
            # used for predict)
            if "env" in data:
                env = data["env"]
        ...
        if pytorch_variables is not None:
            for name in pytorch_variables:
                recursive_setattr(
                    her_model.model, name, pytorch_variables[name])
    ...
    def load_replay_buffer(self,
                           path: Union[str,
                                       pathlib.Path,
                                       io.BufferedIOBase],
                           truncate_last_trajectory: bool = True) -> None:
        """
        ...
        if self.online_sampling:
            ...
            if truncate_last_trajectory and current_idx > 0:
                ...
                self.replay_buffer.buffer["done"][pos][current_idx -
                                                       1] = np.array([True], dtype=np.float32)
                # reset current transition index
                self.replay_buffer.current_idx = 0
                # increment episode counter
                self.replay_buffer.pos = (
                    self.replay_buffer.pos + 1) % self.replay_buffer.max_episode_stored
